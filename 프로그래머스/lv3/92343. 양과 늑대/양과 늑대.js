function solution(info, edges) {
  ...
  const dfs = (current, next, sheep, wolves) => {
    ...
    if (tree[current].length) {
      connectedNodeList = [...connectedNodeList, ...tree[current]];
    }
    connectedNodeList = [
      ...connectedNodeList.slice(0, currentIndex),
      ...connectedNodeList.slice(currentIndex + 1),
    ];
