...
// 2. 그러면 현재 숫자를 기준으로, 자기 자신을 제외한 가장 큰 소수를 찾으면? (N의 M배수인 경우 N을 찾아야 함)
// 3. 근데 생각보다 범주가 너무 크기 때문에 에라토스테네스의 체를 채택해보자.
...
const solution = (begin, end) => {
  ...
  const getCurrentBlockNumber = (index) => {
    ...
    for (let i = 2; i <= Math.sqrt(index); i++) {
      // 만약 해당 수가 index의 약수라면, 나눈 나머지를
      if (index % i === 0) {
        currentBlockNum = i;

        // number의 약수인 i를 나눈 값이 천만보다 작다면, 해당 수가 가장 큰 수다.
